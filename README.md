...
请执行项目根目录下的 `init_mysql_v8.sql` 或 `init_mysql_v5.sql` ，创建表结构和基础数据。
...
- **关于SQL初始化脚本**
  ...
  自2026年5月起，SQL初始化脚本分为如下两个，请根据实际情况择选：

  - `init_mysql_v8.sql`：用于MySQL 8.0版本的初始化脚本；
  - `init_mysql_v5.sql`：用于MySQL 5.0版本的初始化脚本，即原来的 `init.sql`。	
